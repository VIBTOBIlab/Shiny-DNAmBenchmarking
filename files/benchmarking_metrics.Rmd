...
We considered 3 different key metrics: the root-mean-squared error (RMSE), the area under the curve (AUC-ROC) and the Spearman's rank correlation coefficient (ρ {\displaystyle \rho }). To create an overall benchmarking score against which to compare the deconvolution tools, we min-max scaled the metrics and computed the geometric mean of the three metrics to obtain the final benchmarking scores. Finally, we ranked the tools based on these scores.
    ...
    for (fraction in unique_fractions) {
      ...
      plot_data <- bench %>%
        filter(depth == seqdepth & expected_fraction == fraction &
               collapse_approach == approach) %>% group_by(DMRtool)
...
We will investigate and compute the LoD by the statistical significance of a one-sided Welch’s test between the estimated tumor fractions of the samples at every dilution level (e.g. 1%, 5% etc) and the control samples (0% tumor fraction).


```{r}
# Create the main directory to save the plots
output_dir <- paste0(dir_path, "plots/LoD")
if (!dir.exists(output_dir)) dir.create(output_dir)
...
unique_fractions <- unique(bench$expected_fraction)
unique_fractions <- subset(unique_fractions, unique_fractions != 0)
my_comparisons <- lapply(sort(unique_fractions), function(fraction) c("0", as.character(fraction)))
...
for (seqdepth in unique_depths) {
  ...
  for (approach in unique_approaches) {
    ...
    for (dmrtool in unique_dmrtools) {
      ...
      for (deconv in unique(bench$tool)) {
          filt_df <- bench %>%
          ...
          # filt_df2 <- filt_df
          # filt_df2[filt_df2$expected_fraction==0.0001,"expected_fraction"] <- 0.6
          stats <- compare_means(nbl ~ expected_fraction, 
                             data = filt_df,
                             #method = "t.test",
                             method = "wilcox.test",
                             ref.group = "0",  # Reference group set to "0"
                             p.adjust.method = "BH") 
               
          p <- filt_df %>%
            group_by(tool) %>%
            ggplot(aes(x = as.factor(expected_fraction), y = nbl)) +
            geom_boxplot() +
            labs(
              title = paste("LoD | Tool:", deconv,"| DMR tool:", dmrtool,"| Approach:",approach,"| Depth:",seqdepth),
              x = "",
            ...
            stat_compare_means(comparisons = my_comparisons,
                                   #method = "t.test",
                               method = "wilcox.test",
                                   method.args = list(alternative = "less", var.equal = F),
                                   label = "p.format")
                                   #label = "p.signif")
          ...
          # Save the plot as an image
          output_file <- file.path(approach_dir, paste0("LoD_",deconv,"_depth_", seqdepth, "_", approach, "_", dmrtool, ".png"))
          ggsave(output_file, plot = p, width = 8, height = 6)
      }
    }
  }
