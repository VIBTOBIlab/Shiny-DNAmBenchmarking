...
We considered 3 different key metrics: the root-mean-squared error (RMSE), the Jensen--Shannon divergence (JSD) and the Spearman's rank correlation coefficient (ρ {\displaystyle \rho }). To create an overall benchmarking score against which to compare the deconvolution tools, we min-max scaled the metrics and computed the geometric mean of the three metrics to obtain the final benchmarking scores. Finally, we ranked the tools based on these scores.
...
for (fraction in unique(fractions)) {
  df <- bench %>%
    filter(expected_fraction %in% c(0,fraction) & DMRtool == "DMRfinder")
...
# Create a directory to save the plots (optional)
output_dir <- "C:/Users/Sofie/OneDrive - UGent/Documents/Projects/DecoNFlow_benchmarking/plots/funkyheatmaps"
if (!dir.exists(output_dir)) dir.create(output_dir)
