# ==============================================================================
# rrbs_cl.R – UI and Server logic for RRBS - CL benchmarking tab
...
rrbsclTabUI <- function(id, label = "RRBS-CL") {
  ...
  tabPanel(
    title = label, 
    h3("Benchmarking plots: RRBS-CL", style = "font-weight: bold;"),
    ...
    tags$div(id = "lod_rrbscl", 
             h4("Limit of detection (LoD)")),
    p("The plot shows predicted tumoral fractions across increasing expected fumoral fractions for a selected tool and DMR method, with boxplots and Wilcoxon test significance markers comparing each level to 0 to assess the lowest fraction at which signal becomes statistically distinguishable from noise."
    ),
...
moduleServer(id, function(input, output, session) {
  ...
  output$download_rrbscl_df <- downloadHandler(
    filename = function() {
      paste0("rrbs_cl", "_", Sys.Date(), ".csv", sep = "")
    },
