# ==============================================================================
# cfrrbs_invitro.R – UI and Server logic for RRBS benchmarking tab
...
cfrrbsinvitroTabUI <- function(id, label = "cfRRBS in-vitro") {
  ns <- NS(id)
  tabPanel(
    title = label, 

    #fluidPage(
    h3("Benchmarking plots: cfRRBS in-vitro", style = "font-weight: bold;"),
    ...
    tags$div(class = "toc-container",
             ...
             tags$ul(class = "toc-list",
                     tags$li(tags$a(href = "#boxplots_cfrrbsinvitro", "Boxplots of the predictions for each tumoral fraction")),
                     # tags$li(tags$a(href = "#nrmse_cfrrbsinvitro", "Performance (NRMSE)")),
                     # tags$li(tags$a(href = "#heatmap_cfrrbsinvitro", "Heatmap of expected tumoral fraction vs deconvolution tools")),
                     # tags$li(tags$a(href = "#aucroc_cfrrbsinvitro", "AUC-ROC at different tumoral fractions")),
                     tags$li(tags$a(href = "#tools-rmse_cfrrbsinvitro", "Tools RMSE")),
                     tags$li(tags$a(href = "#final_cfrrbsinvitro", "Final ranking of the tools"))
                     # tags$li(tags$a(href = "#lod_cfrrbsinvitro", "Limit of detection"))
             )
    ),
    tags$hr(), br(),
    ...
    ############################################################################
    # Boxplots section
    tags$div(id = "boxplots_cfrrbsinvitro",
    ...
    sidebarLayout(
      ...
      mainPanel(width = 9,
                ...
                downloadButton(ns("download_cfrrbsinvitro_df"), "Download cfRRBS in-vitro"),
    ...
    ############################################################################
    # NRMSE section
    # tags$div(id = "nrmse_cfrrbsinvitro", 
    ...
    ############################################################################
    # Heatmap section
    # tags$div(id = "heatmap_cfrrbsinvitro", 
    #          h4("Heatmap of expected tumoral fraction vs deconvolution tools")),
    # p("The plot shows log-scaled NRMSE values (as both tile color and rounded text) for each deconvolution tool across expected tumoral fractions, with tools ranked left-to-right by their NRMSE at expected TF = 0.048, where lower values indicate better performance."
    #   ),
    # sidebarLayout(
    #   sidebarPanel(width = 3,
    #                selectInput(
    ...
    # tags$div(id = "aucroc_cfrrbsinvitro", 
    ...
    tags$div(id = "tools-rmse_cfrrbsinvitro", 
             h4("Tools RMSE")),
    ...
    tags$div(id = "final_cfrrbsinvitro", 
    ...
    #   sidebarPanel(width = 3),
    #   mainPanel(width = 9,
    #             withSpinner(plotOutput(ns("rank_static"), height = "900px")),
    #             br(),
...
cfrrbsinvitroTabServer <- function(id) {
moduleServer(id, function(input, output, session) {
  ## 1. Filter dataset
  bench <- subset(tot_bench, 
                  seq_method == "cfrrbs" & mixture_type == "in_vitro" &
                    expected_tf < 0.5)
  ...
  output$download_cfrrbsinvitro_df <- downloadHandler(
    filename = function() {
      paste0("cfrrbs_invitro", "_", Sys.Date(), ".csv", sep = "")
    },
  ...
  updateSelectInput(session, "boxplot_tumortype_select",
                    ...
                    selected = if ("NBL" %in% bench$tumor_type) "NBL" else sort(unique(bench$tumor_type))[1] )

  updateSelectInput(session, "boxplot_exptf_select",
                    choices = sort(unique(bench$expected_tf[bench$expected_tf != 0])),
                    selected = if (0.064164744 %in% bench$expected_tf) 0.064164744 else sort(unique(bench$expected_tf[bench$expected_tf != 0]))[1] )
  ...
  filtered_data_boxplot <- reactive({
    req(input$boxplot_tumortype_select, input$boxplot_exptf_select,
  ...
  create_boxplot_TF <- function(data, expected_tf ) {
    ...
    ggplot(data, aes(x = deconv_tool, y = predicted_tf, fill = dmr_tool, color = dmr_tool)) +
      ...
      custom_color_manual + custom_fill_manual + theme(axis.text.x = element_text(angle = 45, hjust = 1)) 
  }
  ...
  output$boxplot_TF <- renderPlotly({
    ...
    req(nrow(data) > 0)
    plot <- create_boxplot_TF(data, as.numeric(input$boxplot_exptf_select))
    ggplotly(plot) %>%
      layout(
        boxmode = "group"
        ) %>%
      config(toImageButtonOptions = list(format = "png",
                                         filename = paste0("boxplot_",input$boxplot_tumortype_select,"_fraction_", input$boxplot_exptf_select, "_", Sys.Date())
      ))
  })
  ...
  output$download_boxplot_TF_df <- downloadHandler(
    filename = function() {
      paste0("boxplot_",input$boxplot_tumortype_select, "_fraction_", input$boxplot_exptf_select, "_", Sys.Date(), ".csv", sep = "")
    },
    content = function(file) {
      data <- filtered_data_boxplot()
      req(nrow(data) > 0)
  ...
  #   req(input$nrmse_tumortype_select, input$nrmse_deconvtool_select, input$nrmse_dmrtools_select)
  #   data <- bench %>%
  ...
  #     custom_color_manual + theme(axis.text.x = element_text(angle = 45, vjust = 0.5, hjust=1))
  # }
  # 
  ...
  #                                        filename = paste0("NRMSE_",input$nrmse_deconvtool_select, "_", input$nrmse_tumortype_select, "_", Sys.Date()))
  #     )
  #   
  # })
  # 
  ...
  #   filename = function() {
  #     paste0("NRMSE_",input$nrmse_deconvtool_select, "_", input$nrmse_tumortype_select, "_", Sys.Date(),".csv", sep = "")
  #   },
  #   content = function(file) {
  #     data <- filtered_data_nrmse()
  #     req(nrow(data) > 0)  
  ...
  #   downloadHandler(
  #     filename = function() paste0("heatmap_tools_", input$heatmap_dmrtool_select, "_", input$heatmap_tumortype_select, "_",Sys.Date(), ".", ext, sep = ""),
  #     content = function(file) {
  #       plot_data <- create_heatmap_data()
  ...
  #   filename = function() paste0("heatmap_tools_", input$heatmap_dmrtool_select, "_", input$heatmap_tumortype_select, "_", Sys.Date(), ".csv", sep = ""),
  #   content = function(file) {
  #     plot_data <- create_heatmap_data()
  ...
  #   fractions <- c(0, 0.007285974, 0.023715415, 0.048, 0.084548105, 0.21067934, 0.404963575)
  ...
  #   downloadHandler(
  #     filename = function() paste0("auc_", input$aucroc_complete_tumortype_select, "_",input$aucroc_complete_dmrtool_select,"_" ,Sys.Date(), ".", ext, sep=""),
  #     content = function(file) {
  ...
  #   filename = function() {
  #     paste0("auc_", input$aucroc_complete_tumortype_select, "_", input$aucroc_complete_dmrtool_select,"_" ,Sys.Date(), ".csv", sep="")
  #   },
  #   content = function(file) {
  #     req(input$aucroc_complete_deconvtools_select, input$aucroc_complete_dmrtool_select)
  #     aucroc_complete_data <- create_aucroc_complete_data()
  ...
  #     config(toImageButtonOptions = list(format = "svg",
  #                                        filename = paste0("auc_", input$aucroc_tumortype_select, "_", input$aucroc_deconvtool_select, "_", input$aucroc_dmrtool_select, "_", Sys.Date())
  #     ))
  # })
  # 
  ...
  #   filename = function() {
  #     paste0("auc_",  input$aucroc_tumortype_select, "_", input$aucroc_deconvtool_select, "_", input$aucroc_dmrtool_select, "_", Sys.Date(), ".csv", sep="")
  #   },
  #   content = function(file) {
  #     aucroc_data <- create_aucroc_data()
  #     req(nrow(aucroc_data) > 0)  
  ...
  output$rmse_comparison <- renderPlotly({
    ...
    ggplotly(plot, tooltip = "text") %>% # Convert ggplot to interactive plotly
      config(toImageButtonOptions = list(format = c("svg","pdf"),
                                         filename = paste0("ranking_deconvtools_", input$rmse_comparison_tumortype_select, "_fraction_", as.numeric(input$rmse_comparison_expectedtf_select),"_", Sys.Date())
      ))
  })
  
  # Save dataframe rmse comparison plot as csv
  output$download_rmse_comparison_df <- downloadHandler(
    filename = function() paste0("ranking_deconvtools_", input$rmse_comparison_tumortype_select, "_fraction_", as.numeric(input$rmse_comparison_expectedtf_select),"_", Sys.Date(), ".csv", sep = ""),
    content = function(file) {
      data <- filtered_data_rmse_comparison() %>%
        group_by(deconv_tool, dmr_tool) %>%
        summarise(RMSE = rmse(expected_tf, predicted_tf), .groups = "drop") %>%
  ...
  normalize_metrics <- reactive({ 
    ...
    for (dmr_tool in input$rank_dmrtools_select) {
      ...
      normalized_list[[key]] <- tmp[, c("deconv_tool", "dmr_tool", 
  ...
  output$rank <- renderPlotly({
    ...
    ggplotly(plot, tooltip = "text") %>% # Convert ggplot to interactive plotly
      config(toImageButtonOptions = list(format = "svg",
                                         filename = paste0("rank_",input$rank_tumortype_select,"_",input$rank_metric_select,"_",Sys.Date())
      ))
  })
  ...
  output$download_rank_df <- downloadHandler(
    filename = function() {
      paste0("rank_",input$rank_tumortype_select,"_",input$rank_metric_select,"_",Sys.Date(), ".csv", sep = "")
    },
    content = function(file) {
      merged_metrics <- merge_metrics_rank()
      normalized_df <- normalize_metrics()
